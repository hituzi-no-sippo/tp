#!/usr/bin/env sh
#
# @(#) v0.1.0 2023-08-31T19:28:23+09:00
...
main() {
  ...
  for path in $non_binary_files; do
    ...
    executable_scripts_without_shebang="$executable_scripts_without_shebang $path"
  done

  if [ "$executable_scripts_without_shebang" = '' ]; then
    exit 0
  fi

  echo "$executable_scripts_without_shebang" | cut --character 2-
